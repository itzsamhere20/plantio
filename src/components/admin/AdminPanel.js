...
const li=' px-2 md:px-4 w-[90%] py-2 rounded-md hover:bg-gray-200 focus:bg-gray-200 hover:cursor-pointer text-[13.8px] md:text-[14px]  text-[#7c7d7c]'
  ...
  return (
    <div className='flex flex-col w-[100%] h-auto ' id='admin'>
        ...
        <div className='flex fixed z-[10] top-[150px] left-[180px] rounded-full p-2 border-[1px] border-gray-400 text-gray-600 bg-gray-100 transition-all duration-300 ease-in-out' style={{left:hidemenu?'10px':'183px', rotate:hidemenu?'0deg':'180deg'}} onClick={()=>sethidemenu(!hidemenu)}><AiOutlineRight/></div>


        {/* --------------side menu's---------- */}

            <div className='block overflow-auto fixed flex-col  w-[200px] bottom-0 pt-[70px] h-[100vh] shadow-[0px_0px_7px_0px_#c0c4c1] bg-gray-100 z-[9] transition-all duration-300' style={{marginLeft:hidemenu?'-300px':'0'}} id='sidemenu'>
            ...
            <li className={li} onClick={()=>setPage(<AdminAbout/>)}>About us</li>
            <li className={li} onClick={()=>setPage(<><ShopNow></ShopNow><AdminShopNow/></>)}>Shop Now</li>
            <li className={li} onClick={()=>setPage(<AdminNewArrivals/>)}>New Arrivals</li>
            ...
            <li className={li} onClick={()=>setPage(<AdminSocialMedia/>)}>Social media</li>
